...
namespace SensorMonitorClient
{
    public class UserInputsVM : INotifyPropertyChanged
    {
        ...
        {
            // Helicopter Type
            helicopterType = (HelicopterType)Enum.Parse(typeof(HelicopterType), config.Read(ConfigKey.HelicopterType));

            // Helideck Category
            helideckCategory = (HelideckCategory)Enum.Parse(typeof(HelideckCategory), config.Read(ConfigKey.HelideckCategory));

            // Day / Night
            dayNight = (DayNight)Enum.Parse(typeof(DayNight), config.Read(ConfigKey.DayNight));
